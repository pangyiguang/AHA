...
class String {
    ...
    /**
     * 检测字符串的长度是否在指定的范围（utf8），这个是长角半角一起算
     * @param string $str       检测的字符串
     * @param integer $min      最小长度
     * @param integer $max      最大长度
     * @return boolean
     */
    static function checkMbUtf8StrLen($str,$min=0,$max=10) {
        $length = strlen($str); //总字长
        $length2 = mb_strlen($str, 'UTF-8'); //utf8模式总字长
        if ($length === $length2) {//半角的
            if ($length < $min || $length > $max)
                return false;
        }elseif ($length % 3 === 0 && $length / $length2 === 3) {//全角的
            if ($length2 < $min || $length2 > $max)
                return false;
        }else {//混合的
            $china = ($length - $length2) / 2;
            $english = $length2 - $china;
            $all = $china + ($english / 2);
            if ($all > $max || $all < $min)
                return false;
        }
     ...
     * @param int $len        截取长度
     * @param string $etc     省略表示符
     * @return string 
     */
    static function utfSubstr($string, $len = 0, $etc = '') {
        $str = $string;
        if ($len == 0) {
            return $str;
        }
     ...
     * @param int $type         1简体转繁体：0繁体转简体
     * @param string $charset   utf-8：gb2312
     * @return string 
     */
    static function simcn2tracn($data, $type = 1, $charset = 'utf-8') {
     ...
     * @param int $numeric  0不全为数字，1全为数字
     * @return string 
     */
